...
├── 06-ビジネス・企画/              # ⚠️ CareCheck Assistサンプル
├── 07-技術仕様/   # ⚠️ 一部CareCheck Assist固有
└── examples/                    # 実装例・サンプル集
```

### ⚠️ 重要な注意事項
- `06-ビジネス・企画/`と`07-技術仕様/`の一部は、特定プロダクト（CareCheck Assist）のサンプルです
...
ユーザー: 「認証機能を実装して」
AI: 以下を確認します：
1. .ai-guide.md の認証セクション
2. docs/03-セキュリティ/ の認証ガイド
3. 実装後、AI_RULES.md のテストルール
```
